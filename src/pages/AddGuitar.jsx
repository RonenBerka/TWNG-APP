...
const STEPS = [
  { num: 1, label: "Photos", icon: "📸", required: true },
  { num: 2, label: "Basic Info", icon: "🎸", required: true },
  { num: 3, label: "Specs", icon: "📋", required: false },
  { num: 4, label: "Story", icon: "📖", required: false },
  { num: 5, label: "Timeline", icon: "⏱️", required: false },
  { num: 6, label: "Review", icon: "👁️", required: true },
];
...
const BRANDS = [
  ...
  "Alvarez", "Washburn", "Breedlove", "Larrivée", "Collings", "Bourgeois",
  ...
  // Bass Specialists
  "Warwick", "Lakland", "Spector", "Höfner", "Dingwall", "Sadowsky", "Fodera",
...
// Smart client-side analysis function — extracts info from filenames and provides intelligent defaults
const analyzeGuitarFromPhotos = (photos) => {
  const result = { ...EMPTY_RESULT };

  // Extract clues from filenames (e.g., "fender-strat.jpg" → brand: Fender, model: Stratocaster)
...
const AI_RESULT = {
  ...
  notes: "Demo mode — deploy the Edge Function for real AI identification.",
};
  ...
  return (
    <div>
      ...
      >
        ...
        <p style={{ fontSize: "13px", color: T.txtM }}>JPG, PNG, WebP · Max 10MB per image · {photos.length}/20</p>
      </div>
...
function StepBasicInfo({ data, setData, onSerialResult }) {
  ...
  const handleDecodeSerial = async () => {
    ...
    try {
      ...
      if (result.success && result.decoded?.year && !data.year) {
        setData({ ...data, year: result.decoded.year.toString() });
      }
      ...
      if (result.success && result.decoded?.brand && !data.brand && SUPPORTED_BRANDS.includes(result.decoded.brand)) {
        setData({ ...data, brand: result.decoded.brand });
      }
    } catch (err) {
    ...
    <div>
      ...
      <FormField label="Brand" required>
        <SelectInput
          ...
          options={BRANDS} placeholder="Select a brand..."
        />
      </FormField>
                ...
                {serialResult.tips && serialResult.tips.length > 0 && (
                  <div style={{ fontSize: "12px", color: T.txtM, marginTop: "8px", paddingTop: "8px", borderTop: `1px solid ${T.border}` }}>
                    ...
                    {serialResult.tips.map((tip, i) => (
                      <p key={i} style={{ margin: "2px 0" }}>• {tip}</p>
                    ))}
                  </div>
                )}
              </>
...
function StepSpecs({ data, setData }) {
  ...
  return (
    <div>
      ...
      <SpecGroup title="Body" icon="🎸">
        <FormField label="Body Type" hint="Shape of the guitar body">
          <SelectInput
            value={data.bodyType} onChange={e => setData({ ...data, bodyType: e.target.value })}
            options={BODY_TYPES} placeholder="Select type..."
          />
        </FormField>
        <FormField label="Body Wood" hint="Primary wood used">
          <SelectInput
            value={data.bodyWood} onChange={e => setData({ ...data, bodyWood: e.target.value })}
            options={WOODS} placeholder="Select wood..."
          />
        </FormField>
        <FormField label="Top Wood" hint="Top veneer (if applicable)">
          <SelectInput
            value={data.topWood} onChange={e => setData({ ...data, topWood: e.target.value })}
            options={WOODS} placeholder="Select wood..."
          />
        </FormField>
        <FormField label="Finish">
          <SelectInput
            value={data.finish} onChange={e => setData({ ...data, finish: e.target.value })}
      ...
      <SpecGroup title="Neck" icon="🔗">
        <FormField label="Neck Wood">
          <SelectInput
            value={data.neckWood} onChange={e => setData({ ...data, neckWood: e.target.value })}
            options={WOODS} placeholder="Select wood..."
          />
        </FormField>
        <FormField label="Fretboard Wood">
          <SelectInput
            value={data.fretboardWood} onChange={e => setData({ ...data, fretboardWood: e.target.value })}
            options={["Rosewood", "Ebony", "Maple", "Pau Ferro", "Pao Ferro", "Walnut", "Other"]}
            placeholder="Select wood..."
          />
        </FormField>
        <FormField label="Neck Profile">
          <TextInput
            value={data.neckProfile} onChange={e => setData({ ...data, neckProfile: e.target.value })}
            placeholder="e.g., C-shape, D-shape"
          />
        </FormField>
        <FormField label="Scale Length">
          <TextInput
            value={data.scaleLength} onChange={e => setData({ ...data, scaleLength: e.target.value })}
            placeholder="e.g., 24.75 or 25.5"
          />
        </FormField>
        <FormField label="Number of Frets">
          <TextInput
            value={data.frets} onChange={e => setData({ ...data, frets: e.target.value })}
            placeholder="e.g., 22 or 24" type="number"
          />
        </FormField>
      </SpecGroup>

      <SpecGroup title="Electronics" icon="⚡">
        <FormField label="Pickup Configuration">
          <TextInput
            value={data.pickupConfig} onChange={e => setData({ ...data, pickupConfig: e.target.value })}
            placeholder="e.g., HH, SSS, HSS"
          />
        </FormField>
        <FormField label="Pickups">
          <TextInput
            value={data.pickups} onChange={e => setData({ ...data, pickups: e.target.value })}
            placeholder="e.g., Seymour Duncan SH-4 JB"
          />
        </FormField>
        <FormField label="Controls">
          <TextInput
            value={data.controls} onChange={e => setData({ ...data, controls: e.target.value })}
            placeholder="e.g., 2 Volume, 1 Tone"
          />
        </FormField>
        <FormField label="Wiring Modifications">
          <TextArea
            value={data.wiringMods} onChange={e => setData({ ...data, wiringMods: e.target.value })}
            placeholder="Describe any custom wiring..." rows={3}
          />
        </FormField>
      </SpecGroup>

      <SpecGroup title="Hardware" icon="🔧">
        <FormField label="Bridge Type">
          <SelectInput
            value={data.bridgeType} onChange={e => setData({ ...data, bridgeType: e.target.value })}
            options={BRIDGE_TYPES} placeholder="Select bridge type..."
          />
        </FormField>
        <FormField label="Bridge Model">
          <TextInput
            value={data.bridgeModel} onChange={e => setData({ ...data, bridgeModel: e.target.value })}
            placeholder="e.g., TonePros ABR-1"
          />
        </FormField>
        <FormField label="Tuners">
          <TextInput
            value={data.tuners} onChange={e => setData({ ...data, tuners: e.target.value })}
            placeholder="e.g., Grover Roto-Matic"
          />
        </FormField>
        <FormField label="Nut Material">
          <SelectInput
            value={data.nutMaterial} onChange={e => setData({ ...data, nutMaterial: e.target.value })}
            options={["Bone", "TUSQ", "Plastic", "Ivory", "Other"]} placeholder="Select material..."
          />
        </FormField>
        <FormField label="Hardware Finish">
          <SelectInput
            value={data.hardwareFinish} onChange={e => setData({ ...data, hardwareFinish: e.target.value })}
        ...
        <div style={{ flex: 1 }}>
          {!recording && !transcribing && (
            <p style={{ fontSize: "13px", color: T.txt2 }}>
              Tell your guitar's story — we'll transcribe it for you
            </p>
          )}
          {recording && (
            <div>
    ...
    <div>
      ...
      <div style={{ background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: "12px", padding: "16px", marginBottom: "24px" }}>
        <p style={{ fontSize: "13px", color: T.txtM, lineHeight: 1.6 }}>
          ✓ All essential information complete<br />
          ✓ Ready to publish to your collection<br />
          ✓ You can edit anytime from your library
        </p>
      </div>
    </div>
  );
...
function AddGuitarManual({ initialData }) {
    ...
    try {
      ...
      for (let i = 0; i < formData.photos.length; i++) {
        ...
        if (uploadError) {
          ...
          // Continue with other photos — don't block the whole submission
          continue;
        }
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 2: return <StepBasicInfo data={formData} setData={setFormData} onSerialResult={setSerialResult} />;
...
function MagicAddAI({ onComplete, onSwitchManual }) {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // 3. Got real AI results — validate shape and use them
  ...
  // Progress animation — runs during processing phase
  ...
  const handleContinueToForm = () => {
    ...
    // Handle year ranges like "2020-2024" — take the first year
      ...
      >
        {uploadedPhotos.length === 0 ? (
          <>
            ...
            <p style={{ fontSize: "13px", color: T.txtM }}>JPG, PNG, WebP · Max 10MB per image</p>
          </>
        ) : (
          <>
            ...
            <p style={{ fontSize: "12px", color: T.txtM }}>{uploadedPhotos.length}/5 photos · Click to add more</p>
          </>
        )}
      </div>
      ...
      {analysisSource === "empty" && (
        <div style={{ background: "#3B82F615", border: `1px solid #3B82F640`, borderRadius: "12px", padding: "16px", marginBottom: "16px" }}>
          ...
          <p style={{ fontSize: "12px", color: T.txt2, lineHeight: 1.6 }}>
            The AI backend couldn't identify your guitar. Please fill in the details manually. We're working to improve guitar identification — your feedback helps!
          </p>
        </div>
      )}
...
export default function TWNGAddGuitar() {
  ...
  // Called when Magic Add analysis is complete — switch to manual form with pre-filled data
      ...
      <header style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: "4px", background: T.bgCard, padding: "5px", borderRadius: "12px", border: `1px solid ${T.border}` }}>
            {[
              { key: "manual", label: "Add Guitar (Manual)", icon: "✏️" },
              { key: "magic", label: "Magic Add (AI)", icon: "✨" }
            ].map(m => (
              <button
                key={m.key}
